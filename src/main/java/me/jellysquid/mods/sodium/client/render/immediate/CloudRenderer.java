...
public class CloudRenderer {
    ...
    private float cloudSizeX, cloudSizeZ;
                       ...
                       float tickDelta)
    {
        ...
        final float translateX = (float) (cloudCenterX - (centerCellX * this.cloudSizeX));
        final float translateZ = (float) (cloudCenterZ - (centerCellZ * this.cloudSizeZ));
    ...
    private @NotNull CloudGeometry rebuildGeometry(@Nullable CloudGeometry existingGeometry,
    ...
    private void addCellGeometryToBuffer(VertexBufferWriter writer,
                                                CloudTextureData textureData,
    ...
    private void emitCellGeometry2D(VertexBufferWriter writer, int faces, int color, float x, float z) {
    ...
    private void emitCellGeometry3D(VertexBufferWriter writer, int faces, int color, float x, float z) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
    ...
    public void reloadTextures(ResourceProvider factory) {
        ...
        float width = shaderMod ? 256 : textureData.width;
        float height = shaderMod ? 256 : textureData.height;

        this.cloudSizeX = MAX_SINGLE_CLOUD_SIZE / width;
        this.cloudSizeZ = MAX_SINGLE_CLOUD_SIZE / height;

        try {
            this.shaderProgram = new ShaderInstance(factory, "clouds", DefaultVertexFormat.POSITION_COLOR);
        } catch (IOException e) {
